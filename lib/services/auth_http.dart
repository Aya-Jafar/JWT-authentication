...
class AuthService {
  ...
  static Future<Account?> signInHttp(
      {required String email, required String password}) async {
    try {
      if (email.isNotEmpty && password.isNotEmpty) {
        final response = await http.post(
          Uri.parse('https://ayajafar001.pythonanywhere.com/api/auth/signin/'),
        ...
        if (response.statusCode == 200) {
          ...
          Get.offAll(() => HomePage(account: Account.fromJson(accountOut)));
  ...
  static Future<Account?> signUpHttp(
    ...
    try {
      if ([userName, email, password1, password2, phoneNumber]
        ...
        final response = await http.post(
          Uri.parse('https://ayajafar001.pythonanywhere.com/api/auth/signup/'),
          body: jsonEncode({
            "user_name": userName,
            "email": email,
            "password1": password1,
        ...
        if (response.statusCode == 201) {
          ...
          Get.to(() => HomePage(account: Account.fromJson(accountOut)));
